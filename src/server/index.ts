...
export class Globe extends Server {
	...
	async onMessage(conn: Connection, message: ArrayBuffer | ArrayBufferView | string) {
		...
		switch (msg.type) {
			case "debug-enable":
				this.debugConnections.add(conn.id);
				this.sendTo(conn, { type: "debug-status", enabled: true });
				this.sync();
				break;
			case "hello":
	...
	private handleDebugScreenMessage(conn: Connection, rawText: string) {
		if (!this.debugConnections.has(conn.id)) return;
		const text = String(rawText ?? "").trim().slice(0, 240);
		if (!text) return;
		this.broadcast(JSON.stringify({ type: "debug-screen-message", text, from: "Developer" } satisfies OutgoingMessage));
	}
